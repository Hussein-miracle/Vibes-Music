...
import {selectMusicsData,selectCurrentMusic } from "../../redux/musics/musics.selectors";
import {selectBgMode} from "../../redux/user/user.selectors";
import img from "../../assets/images/Love_Nwantinti.jpg"
import svgContainer from "../../assets/icons/sprite.svg";
import "./music-player.styles.scss";



const MusicPlayer = function({light,currentMusic,musics,song}) {
  const {id,imgURL,srcURL,title,artists} = song;
...
const renderPause = () => {
    let pauseBtn = null;
    if(light){
      pauseBtn = <svg className="music-control__play icon-music-controls icon-white-2" width="9" height="15" viewBox="0 0 9 15" fill="none" xmlns="http://www.w3.org/2000/svg">
            <path d="M1.58792 0.645264V14.9963M7.9852 0.645264V14.9963" stroke="#091127" strokeWidth="2.00263"/>
            </svg>
    }else{
      pauseBtn = <svg className="music-control__play icon-music-controls icon-white-2" width="14" height="25" viewBox="0 0 14 25" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const renderPlay = () => {
      let playBtn = null;
     if(light){
      playBtn = <svg className="music-control__play icon-music-controls icon-white-2" style={{
        fill:"var(--dark)"
      }}>
